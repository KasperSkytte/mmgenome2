...
mmref <- function(data = NULL, tax.level="Phylum", tax.compare, tax.aggregate = tax.level, summarise = "mean", output = "plot", plot.display = T, plot.sort = T){
  ...
  suppressWarnings(if(tax.compare != "all"){ es <- subset(ess.genes, ess.genes[,tax.level] %in% tax.compare) } else {es <- ess.genes})
  
  ### Convert the dataframe to long format
  em <- melt(es,id.vars=c(colnames(ess.genes)[1:9]), value.name = "Count", variable.name = "HMM")
